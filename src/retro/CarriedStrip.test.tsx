...
describe('CarriedStrip', () => {
  ...
  it('opens expanded and reports progress while items are unreviewed', () => {
    render(
      <CarriedStrip items={[item('done'), item('pending')]} locked={false} onSetStatus={vi.fn()} />,
    );
    expect(screen.getByRole('button', { name: /Last retro/ }).getAttribute('aria-expanded')).toBe(
      'true',
    );
  ...
  it('folds itself away once every item has a status', () => {
    ...
    render(
      <CarriedStrip
        items={[item('done'), item('carried_over')]}
        locked={false}
        onSetStatus={vi.fn()}
      />,
    );
  ...
  it('stays open after you reopen a finished strip', async () => {
    ...
    const { rerender } = render(
      <CarriedStrip items={items} locked={false} onSetStatus={vi.fn()} />,
    );
  ...
  it('names each select after the item it governs', () => {
    ...
    render(
      <CarriedStrip
        items={[card({ text: 'Add staging alerts', status: 'pending' })]}
        locked={false}
        onSetStatus={vi.fn()}
      />,
    );
