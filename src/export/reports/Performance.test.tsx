...
describe('Performance', () => {
  it('names the engineer with an avatar', () => {
    const { container } = render(
      <Performance engineer="Ada Lovelace" sections={[]} warnings={[]} />,
    );
        ...
        sections={[
          {
            ...
            items: [LONG, 'Discuss growth; align on goals.'],
          },
        ]}
        warnings={[]}
      />,
  ...
  it('keeps split fragments in one flat list', () => {
    ...
    const { container } = render(
      <Performance
        engineer="Ada"
        sections={[{ id: 'points', title: 'Points', items: [LONG] }]}
        warnings={[]}
      />,
    );
  ...
  it('draws nothing for a section with no items', () => {
    const { container } = render(
      <Performance
        engineer="Ada"
        sections={[{ id: 'gaps-observed', title: 'Gaps observed', items: [] }]}
        warnings={[]}
      />,
    );
  ...
  it('gives every section a linkable id', () => {
    const { container } = render(
      <Performance
        engineer="Ada"
        sections={[{ id: 'areas-for-improvement', title: 'Areas for improvement', items: ['x'] }]}
        warnings={[]}
      />,
    );
  ...
  it('shows the framework footnote when one was used', () => {
    render(
      <Performance engineer="Ada" sections={[]} footnote="Framework: Dreyfus" warnings={[]} />,
    );
...
describe('Performance — the numbers', () => {
  ...
  it('paints no stat, because whether a figure is good is the reader’s judgement', () => {
    const { container } = render(
      <Performance engineer="Ada" sections={[]} warnings={[]} stats={[STAT]} />,
    );
  ...
  it('degrades an unknown unit to the bare number', () => {
    const { container } = render(
      <Performance
        ...
        sections={[]}
        warnings={[]}
        stats={[{ ...STAT, unit: 'furlongs', value: 9 }]}
      />,
    );
  ...
  it('demotes stats past the headline four to a list rather than dropping them', () => {
    ...
    const { container } = render(
      <Performance engineer="Ada" sections={[]} warnings={[]} stats={many} />,
    );
...
describe('Performance — coverage and evidence', () => {
  ...
  it('names each source the way the rest of the app names it', () => {
    const { container } = render(
      <Performance
        ...
        sections={[]}
        warnings={[]}
        coverage={[{ source: 'analysis', state: 'covered', detail: '' }]}
      />,
    );
  ...
  it('spends a line on a gap and not on a source that explains itself', () => {
    const { container } = render(
      <Performance engineer="Ada" sections={[]} warnings={[]} coverage={COVERAGE} />,
    );
  ...
  it('draws no evidence section for a group with no rows', () => {
    const { container } = render(
      <Performance
        ...
        sections={[]}
        warnings={[]}
        evidence={[{ source: 'code', label: 'Code', note: '', items: [] }]}
      />,
    );
  ...
  it('drops a section that is empty for no stated reason', () => {
    const { container } = render(
      <Performance
        engineer="Ada"
        warnings={[]}
        sections={[{ id: 'gaps', title: 'Gaps observed', items: [] }]}
      />,
    );
    expect(container.textContent).not.toContain('Gaps observed');
  });
  ...
  it('renders an unknown state rather than nothing', () => {
    const { container } = render(
      <Performance
        engineer="Ada"
        warnings={[]}
        sections={[{ id: 'gaps', title: 'Gaps observed', items: [], state: 'sampled' }]}
      />,
    );
  ...
  it('names the window a review covers beside the engineer', () => {
    const { container } = render(
      <Performance
        ...
        sections={[]}
        warnings={[]}
        period={{ start: '2026-01-12', end: '2026-07-12' }}
      />,
    );
